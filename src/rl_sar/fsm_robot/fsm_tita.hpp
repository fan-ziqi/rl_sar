...
public:
    ...
    void Run() override
    {
        for (int i = 0; i < rl.params.num_of_dofs; ++i)
        {
            ...
            fsm_command->motor_command.kd[i] = 1;
...
class TITAFSMFactory : public FSMFactory
{
public:
    TITAFSMFactory(const std::string& initial) : initial_state_(initial) {}
    std::shared_ptr<FSMState> CreateState(void *context, const std::string &state_name) override
    {
        RL *rl = static_cast<RL *>(context);
        if (state_name == "RLFSMStatePassive")
            return std::make_shared<tita_fsm::RLFSMStatePassive>(rl);
...
REGISTER_FSM_FACTORY(TITAFSMFactory, "RLFSMStatePassive")
