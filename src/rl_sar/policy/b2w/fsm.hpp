...
class B2WFSMFactory : public FSMStateFactory
{
public:
    B2WFSMFactory(const std::string& initial) : initialState(initial) {}
    std::shared_ptr<FSMState> createState(void *context, const std::string &stateName) override
    {
        ...
        if (stateName == "RLFSMStateWaiting")
            ...
            return std::make_shared<b2w_fsm::RLFSMStateRL_Navigation>(rl);
        return nullptr;
    }
