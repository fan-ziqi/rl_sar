...
class Go2WFSMFactory : public FSMStateFactory
{
public:
    Go2WFSMFactory(const std::string& initial) : initialState(initial) {}
    std::shared_ptr<FSMState> createState(void *context, const std::string &stateName) override
    {
        ...
        if (stateName == "RLFSMStateWaiting")
            ...
            return std::make_shared<go2w_fsm::RLFSMStateRL_Locomotion>(rl);
        else if (stateName == "RLFSMStateRL_Navigation")
            return std::make_shared<go2w_fsm::RLFSMStateRL_Navigation>(rl);
        return nullptr;
    }
