...
class Gr1t2FSMFactory : public FSMStateFactory
{
public:
    Gr1t2FSMFactory(const std::string& initial) : initialState(initial) {}
    std::shared_ptr<FSMState> createState(void *context, const std::string &stateName) override
    {
        RL *rl = static_cast<RL *>(context);
        if (stateName == "RLFSMStateWaiting")
            return std::make_shared<gr1t2_fsm::RLFSMStateWaiting>(rl);
            ...
            return std::make_shared<gr1t2_fsm::RLFSMStateRL_Navigation>(rl);
        return nullptr;
    }
...
REGISTER_FSM_FACTORY(Gr1t2FSMFactory, "RLFSMStateWaiting")
