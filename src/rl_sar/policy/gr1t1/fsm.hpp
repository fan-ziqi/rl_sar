...
class Gr1t1FSMFactory : public FSMStateFactory
{
public:
    Gr1t1FSMFactory(const std::string& initial) : initialState(initial) {}
    std::shared_ptr<FSMState> createState(void *context, const std::string &stateName) override
    {
        RL *rl = static_cast<RL *>(context);
        if (stateName == "RLFSMStateWaiting")
            return std::make_shared<gr1t1_fsm::RLFSMStateWaiting>(rl);
        else if (stateName == "RLFSMStateGetUp")
            return std::make_shared<gr1t1_fsm::RLFSMStateGetUp>(rl);
        else if (stateName == "RLFSMStateGetDown")
            return std::make_shared<gr1t1_fsm::RLFSMStateGetDown>(rl);
        else if (stateName == "RLFSMStateRL_Locomotion")
            return std::make_shared<gr1t1_fsm::RLFSMStateRL_Locomotion>(rl);
        else if (stateName == "RLFSMStateRL_Navigation")
            return std::make_shared<gr1t1_fsm::RLFSMStateRL_Navigation>(rl);
        return nullptr;
    }
    std::string getType() const override { return "gr1t1"; }
...
REGISTER_FSM_FACTORY(Gr1t1FSMFactory, "RLFSMStateWaiting")
